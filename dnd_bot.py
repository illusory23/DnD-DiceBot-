...
NEUTRAL_CREATURE_TABLE = {
    1: "雪蹄兔（被惊吓可跟踪至附近的浆果丛，获得1d2天口粮，生存检定DC12）（1d2只）",
    2: "野鹿（跟踪可发现附近的野菜或者发现荧光苔丛，自然检定DC15，成功则投掷1d100,1-20则为荧光苔丛，获得2份荧光苔，21-100则为野菜，获得1d2天口粮）（1d5只）",
    3: "雪鸮（夜间活动，跟随可发现岩洞，生存检定DC15）（1d2只）",
    4: "都灵寒鸦（展示友善并喂食1天口粮可为你示警，避开下一次遭遇中的恶意事件，驯养检定DC10）（1只）",
    5: "雪狐（展示友善并喂食1天口粮可引路至附近的特殊发现，驯养检定DC15）（1只）",
    6: "霜鼠（跟踪可发现附近温暖地脉，接下来两次事件中体质检定优势，自然检定DC15）（2d3只）",
    7: "雪蹄兔（被惊吓可跟踪至附近的浆果丛，获得1d2天口粮，生存检定DC12）（1d3只）",
    8: "野鹿（跟踪可发现附近的野菜或者发现荧光苔丛，自然检定DC15，成功则投掷1d100,1-20则为荧光苔丛，获得2份荧光苔，21-100则为野菜，获得1d2天口粮）（1d3只）",
...
FX100_TABLE = _build_interval_table([
    ...
    ((16, 40), "发现猎人小屋（可进入长休/短休，可获得1d4天口粮）"),
...
TABLE_ITEM_ACTIONS = {
    'cl50': {
        ...
        **{i: {"item": "浆果/野菜", "qty_roll": "1d2", "unit": "天口粮"} for i in range(7, 16)},
