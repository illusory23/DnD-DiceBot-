...
NEUTRAL_CREATURE_TABLE = {
    1: "雪蹄兔（被惊吓可跟踪至附近的浆果丛，获得1d2份食物，生存检定DC12）（1d2只）",
    2: "野鹿（跟踪可发现附近的野菜或者发现荧光苔丛，自然检定DC15，成功则投掷1d100,1-20则为荧光苔丛，获得2份荧光苔，21-100则为野菜，获得1d2份食物）（1d5只）",
    ...
    7: "雪蹄兔（被惊吓可跟踪至附近的浆果丛，获得1d2份食物，生存检定DC12）（1d3只）",
    8: "野鹿（跟踪可发现附近的野菜或者发现荧光苔丛，自然检定DC15，成功则投掷1d100,1-20则为荧光苔丛，获得2份荧光苔，21-100则为野菜，获得1d2份食物）（1d3只）",
...
SPECIAL_MATERIAL_TABLE = _build_interval_table([
    ...
    ((26, 45), "松茸（较稀有的食材，可出售；可做陷阱诱饵诱捕部分生物；食用后下次力量或体质检定优势）"),
    ((46, 65), "铁松松脂（易燃，耐烧，可在暴风雪中点燃；可使用一个附赠动作点燃涂抹至武器上，使武器在一场战斗中可额外造成1d6火焰伤害）"),
    ((66, 72), "绒蜂蜜（北境的绒蜂所产出的珍稀食材，可出售；可做陷阱诱饵诱捕部分生物；处理后食用后恢复1d4生命，并且在一段时间内获得寒冷抗性，并且免疫失温或冰冻效果；直接食用有弱毒型，1d3，若结果为2-3则腹泻，下次体质检定劣势）"),
    ((73, 85), "银蛛蛛网（韧性较强，较稀有材料，可制作网兜，可制作陷阱，可出售）"),
    ((86, 95), "霜晶核（蕴含着极其冰冷的能量，可用于制作魔法物品，可用于附魔武器，可出售）"),
    ((96, 100), "寒铁髓（寒铁中最为坚硬，魔力相性最好的部分的精华，可用于制作魔法物品，可出售）"),
])

# 随机矿物 d100 表 (.kw100) — 区间表
MINERAL_TABLE = _build_interval_table([
    ((1, 10), "岩盐（可用于腌制食品，可用于调味，可出售）"),
    ((11, 25), "煤矿（可用于火炉生火；可出售）"),
    ((26, 35), "铜矿（熔炼后可用于制作工具，可出售）"),
    ((36, 50), "铁矿（熔炼后可用于制作工具，可出售）"),
    ((51, 60), "寒铁矿（北境特产铁矿，比普通铁矿更坚硬，魔力相性更好；熔炼后可制作工具，可出售；有概率产出寒铁髓，寒铁中最为坚硬，魔力相性最好的部分的精华；可用于制作魔法物品，可出售；自动投掷1d6，若结果为6则产出寒铁髓）"),
    ((61, 70), "银矿（可出售）"),
    ((71, 75), "金矿（熔炼后可制作饰品，可制作魔法物品，可出售）"),
    ((76, 85), "冰水晶（蓝色晶体，可用于制作魔法物品，可出售）"),
    ((86, 92), "霜晶核（蕴含着极其冰冷的能量；可用于制作魔法物品，可用于附魔武器，可出售）"),
    ((93, 96), "化石（可出售）"),
    ((97, 100), "宝石（可制作饰品，可制作魔法物品，可出售）"),
])
...
ITEM_TABLE = {
    ...
    6: "松茸（较稀有的食材，可出售；可做陷阱诱饵诱捕部分生物；食用后下次力量或体质检定优势）",
    7: "松脂（可点燃，可制作火把）",
    8: "铁松松脂（易燃，耐烧，可在暴风雪中点燃；可使用一个附赠动作点燃涂抹至武器上，使武器在一场战斗中可额外造成1d6火焰伤害）",
    9: "普通兽骨（可打磨制成工具，可出售）",
    10: "霜羽雉的蛋（富含营养的食材，可孵化，可出售）",
    11: "霜羽雉的银羽（霜羽雉最出名的尾羽，可作装饰品，可出售）",
    12: "绒蜂蜜（北境的绒蜂所产出的珍稀食材，可出售；可做陷阱诱饵诱捕部分生物；处理后食用后恢复1d4生命，并且在一段时间内获得寒冷抗性，并且免疫失温或冰冻效果；直接食用有弱毒型，1d3，若结果为2-3则腹泻，下次体质检定劣势）",
    13: "银蛛蛛网（韧性较强，较稀有材料，可制作网兜，可制作陷阱，可出售）",
    14: "矿粒（可出售）",
    15: "岩盐（可用于腌制食品，可用于调味，可出售）",
    16: "煤矿（可用于火炉生火；可出售）",
    17: "铜矿（熔炼后可用于制作工具，可出售）",
    18: "铁矿（熔炼后可用于制作工具，可出售）",
    19: "银矿（可出售）",
    20: "金矿（熔炼后可制作饰品，可制作魔法物品，可出售）",
    21: "寒铁矿（北境特产铁矿，比普通铁矿更坚硬，魔力相性更好；熔炼后可制作工具，可出售；有概率产出寒铁髓，寒铁中最为坚硬，魔力相性最好的部分的精华；可用于制作魔法物品，可出售；自动投掷1d6，若结果为6则产出寒铁髓）",
    22: "寒铁髓（寒铁中最为坚硬，魔力相性最好的部分的精华，可用于制作魔法物品，可出售）",
    23: "霜晶核（蕴含着极其冰冷的能量，可用于制作魔法物品，可用于附魔武器，可出售）",
    24: "冰水晶（蓝色晶体，可用于制作魔法物品，可出售）",
    25: "化石（可出售）",
    26: "宝石（可制作饰品，可制作魔法物品，可出售）",
    27: "钱袋（某个倒霉蛋的遗物，获得1d3金币）",
    28: "巨兽之骨（埋骨于北境的远古巨兽的骨头，可制作魔法物品，可出售）",
    29: "巨兽秘骨（埋骨于北境的远古巨兽的骨头，其中精华在寒冷的淬炼下拥有了优良的魔力相性，可制作魔法物品，可出售）",
}
...
TABLE_ITEM_ACTIONS = {
    'cl50': {
        ...
        **{i: {"item": "浆果/野菜", "qty_roll": "1d2", "unit": "份食物"} for i in range(7, 16)},
...
def _apply_item_action(action: dict, roll_val: int, pre_rolled: dict | None = None) -> str | None:
    """执行物品添加动作，返回提示文本。无活跃角色时仅显示掷骰结果。

    pre_rolled: 可选，{dice_expr: result} 映射，用于复用已掷骰结果避免重复掷骰。
    """
    char = get_active_char()
    result_parts = []
    ...
    if 'coin' in action:
        ...
        if pre_rolled and coin_roll in pre_rolled:
            qty = pre_rolled[coin_roll]
        else:
    ...
    if 'qty_roll' in action:
        ...
        if pre_rolled and qty_roll_expr in pre_rolled:
            qty = pre_rolled[qty_roll_expr]
        else:
    ...
    if not char:
        result_parts.append(f"📦 +{qty}{unit} {item_name}（无活跃角色）")
    else:
    ...
    if bonus:
        ...
        result_parts.append(
            f"   🎲 额外判定: 结果={color_cyan(str(bonus_val))} "
...
def _parse_and_roll_qty(desc: str) -> tuple[str, str]:
    ...
    suffix = f" ×{color_cyan(str(qty_val))}{unit}"
    return clean_desc, suffix


def _roll_dice_in_text(text: str, dice_map: dict | None = None) -> str:
    """将文本中所有 XdY 骰子表达式替换为实际掷骰结果（面向玩家显示）。

    dice_map: 可选，{dice_expr: result} 映射，用于复用已掷骰结果。

    注意：物品效果描述中的骰子（如"1d6火焰伤害"）不会被替换，
    只替换获得数量/价值类骰子（如"获得1d3金币"）。
    """
    if not text:
        return text
    if dice_map is None:
    ...
    # 效果类后缀：骰子后紧跟这些词说明是物品属性，保留不替换
    _EFFECT_SUFFIXES = ('伤害', '火焰', '寒冷', '穿刺', '钝击', '挥砍', '生命', '力竭')

    def _is_effect_dice(end_pos: int) -> bool:
        after = text[end_pos:end_pos + 4]
        return any(after.startswith(s) for s in _EFFECT_SUFFIXES)

    def _replacer(m: _re.Match) -> str:
        dice_expr = m.group(0)
        if _is_effect_dice(m.end()):
            return dice_expr  # 物品属性骰子，保留原文
        if dice_expr in dice_map:
            return str(dice_map[dice_expr])
        try:
            r = roll(dice_expr)
            return str(r.total)
        except Exception:
    ...
    return _re.sub(r'\d+d\d+(?:[+-]\d+)?', _replacer, text)


def _fmt_desc(desc: str, dice_map: dict | None = None) -> str:
    """格式化描述文本，自动投掷生物数量骰子和内嵌骰子表达式。

    dice_map: 可选，用于复用已掷骰结果（避免描述与行动结果不一致）。
    """
    clean, suffix = _parse_and_roll_qty(desc)
    # 对剩余文本中的骰子表达式进行投掷替换
    clean = _roll_dice_in_text(clean, dice_map)
    return clean + suffix


def _preview_action_dice(action: dict) -> dict[str, int]:
    """预掷动作中的骰子，返回 {dice_expr: result} 映射，用于描述文本替换。"""
    dice_map = {}
    if 'coin_roll' in action:
        qty = roll(action['coin_roll']).total
        dice_map[action['coin_roll']] = qty
    if 'qty_roll' in action:
        qty = roll(action['qty_roll']).total
        dice_map[action['qty_roll']] = qty
    return dice_map
...
def _roll_event_table(table_dict: dict, label: str, dice: str, depth: int = 0,
    ...
    if table_key and table_key in TABLE_ITEM_ACTIONS:
        ...
        if roll_val in actions:
            action = actions[roll_val]
            # 预掷动作中的骰子，确保描述和行动结果一致
            pre_rolled = _preview_action_dice(action)
            # 用预掷值重新格式化描述
            if pre_rolled:
                # 重新生成输出行（用预掷值替换描述中的骰子）
                output = f"🎲 {bold(label)} [{dice}: {color_cyan(str(roll_val))}]\n   {_fmt_desc(desc, pre_rolled)}"
            item_msg = _apply_item_action(action, roll_val, pre_rolled)
            if item_msg:
                output += f"\n   {item_msg}"
        ...
        for ri in range(chain_repeat):
            ...
            if sub_table_key in TABLE_ITEM_ACTIONS:
                ...
                if sub_val in sub_actions:
                    sub_pre_rolled = _preview_action_dice(sub_actions[sub_val])
            output += f"\n   ↳ 🎲 {bold(sub_label)} [{sub_dice}: {color_cyan(str(sub_val))}]{repeat_tag}\n      {_fmt_desc(sub_desc, sub_pre_rolled)}"

            if sub_pre_rolled:
                item_msg = _apply_item_action(sub_actions[sub_val], sub_val, sub_pre_rolled)
                if item_msg:
                    output += f"\n      {item_msg}"
        ...
        if sub_sub_key and depth < 1:
            if isinstance(sub_sub_key, dict):
                ...
                for sri in range(sub_repeat):
                    ...
                    sub2_pre_rolled = {}
                    if sub_sub_key in TABLE_ITEM_ACTIONS and sub_sub_val in TABLE_ITEM_ACTIONS[sub_sub_key]:
                        sub2_pre_rolled = _preview_action_dice(TABLE_ITEM_ACTIONS[sub_sub_key][sub_sub_val])
                    output += f"\n      ↳ 🎲 {bold(sub_sub_label)} [{sub_sub_dice}: {color_cyan(str(sub_sub_val))}]{sri_tag}\n         {_fmt_desc(sub_sub_desc, sub2_pre_rolled)}"

                    if sub2_pre_rolled:
                        item_msg = _apply_item_action(TABLE_ITEM_ACTIONS[sub_sub_key][sub_sub_val], sub_sub_val, sub2_pre_rolled)
                        if item_msg:
                            output += f"\n         {item_msg}"
                    ...
                    if sub_sub_sub_key and isinstance(sub_sub_sub_key, str) and sub_sub_sub_key in SUB_TABLE_REGISTRY:
                        ...
                        for s3i in range(sub3_repeat):
                            ...
                            if sub_sub_sub_key in TABLE_ITEM_ACTIONS and sub3_val in TABLE_ITEM_ACTIONS[sub_sub_sub_key]:
                                sub3_pre_rolled = _preview_action_dice(TABLE_ITEM_ACTIONS[sub_sub_sub_key][sub3_val])
                            output += f"\n         ↳ 🎲 {bold(sub3_label)} [{sub3_dice}: {color_cyan(str(sub3_val))}]{s3_tag}\n            {_fmt_desc(sub3_desc, sub3_pre_rolled)}"

                            # item actions for this level
                            if sub3_pre_rolled:
                                item_msg = _apply_item_action(TABLE_ITEM_ACTIONS[sub_sub_sub_key][sub3_val], sub3_val, sub3_pre_rolled)
                                if item_msg:
                                    output += f"\n            {item_msg}"
