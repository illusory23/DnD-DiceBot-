        ...
        (async function boot() {
            ...
            async function renderStatusPool() {
                ...
                list.innerHTML = rows.map(function(r) { return statusPoolRow(r, dm); }).join('');
            }

            function statusPoolRow(r, dm) {
                const sts = r.token ? (r.token.statuses || []) : [];
                const chips = sts.map(function(s) {
                    const dur = (s && s.unit) ? ' <span class="st-rounds">' + fmtStatusShort(s) + '</span>' : ' <span class="st-forever">∞</span>';
                    const del = dm ? ' <span class="st-del" onclick="delTokenStatus(' + r.token.id + ", '" + s.id + "')\" title=\"删除\">×</span>" : '';
                    return '<span class="status-chip">' + escapeHtml(s.name) + dur + del + '</span>';
                }).join('');
                const addBtn = (dm && r.inMap)
                    ? '<span style="display:flex;gap:3px;margin-top:4px;flex-wrap:wrap;">'
                        + '<input class="st-input" id="st-name-' + r.token.id + '" placeholder="状态名" style="flex:1;min-width:64px;">'
                        + '<select class="st-input" id="st-unit-' + r.token.id + '" style="width:60px;" title="单位（回合随战斗轮递减；分钟/小时为静态时长）">'
                        + '<option value="回合">回合</option><option value="分钟">分钟</option><option value="小时">小时</option><option value="">永久</option>'
                        + '</select>'
                        + '<input class="st-input" id="st-value-' + r.token.id + '" type="number" min="1" placeholder="时长" style="width:52px;" title="时长数值（回合/分钟/小时）">'
                ...
                return '<div class="status-pool-row">'
                    + '<div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;">👤 <b>' + escapeHtml(r.label) + '</b> ' + chips + '</div>'
                    + addBtn + hint
                    + '</div>';
            }
