...
def api_dashboard():
    ...
    # 归档数（PostgreSQL chat_archives 行数）
    archive_count = 0
    archive_size = 0
    try:
...
def api_system_info():
    ...
    # 数据库大小（PostgreSQL pg_database_size）
    db_size_mb = 0
    try:
        from sqlalchemy import text as _sa_text
        import web.app as _app
        with _app.db.engine.connect() as _conn:
            row = _conn.execute(_sa_text(
                "SELECT pg_database_size(current_database())")).scalar()
            if row:
                db_size_mb = round(row / 1048576, 2)
    except Exception:
        pass
...
def _admin_load_topics() -> dict:
    """从 PostgreSQL 加载每周话题（2026-08-15 起复用 app.py ORM 入口，
    顺带修复原双套读写无锁竞态）。"""
    import web.app as _app
    return _app._load_topics()


def _admin_save_topics(data: dict):
    import web.app as _app
    _app._save_topics(data)
...
# 备份内容清单: (源相对路径, 是否目录)
# 2026-08-15 PostgreSQL 迁移后：运行数据已入 PG，备份中心改为
# PG 全库导出(pg_dump) + 文件类数据(custom/图层/日志) 组合。
_BACKUP_SOURCES = [
    ('data/custom', True),
    ('maps/layers', True),
    ('logs', True),
...
_RESTORE_MAP = {
    ...
    'layers': 'maps/layers',
    'logs': 'logs',
    'dicebot_pg_dump.sql': 'data/pg_backup/dicebot_pg_dump.sql',
}
...
def _sqlite_backup(src_path: _Path, dst_path: _Path) -> None:
    ...
    2026-08-15 起仅用于历史兼容，当前数据已迁移 PostgreSQL。
    """
...
_PG_RESTORE_ORDER = [
    ...
    'chat_messages', 'chat_archives', 'dm_events', 'dm_published_events',
    'event_stats', 'dice_stats', 'combat_state', 'shared_canvas',
    'topics', 'announcements', 'community_posts', 'workshop_items', 'favorites',
]
_MODEL_BY_TABLE = {
    'users': 'User', 'character_groups': 'CharacterGroup', 'characters': 'Character',
    'abilities': 'Ability', 'skill_proficiencies': 'SkillProficiency',
    'save_proficiencies': 'SaveProficiency', 'spell_slots': 'SpellSlot',
    'death_saves': 'DeathSave', 'weapons': 'Weapon', 'armor': 'Armor', 'coins': 'Coins',
    'prepared_spells': 'PreparedSpell', 'learned_spells': 'LearnedSpell',
    'background_details': 'BackgroundDetail', 'inventory': 'InventoryItem',
    'character_features': 'CharacterFeature', 'north_saves': 'NorthSave',
    'chat_messages': 'ChatMessage', 'chat_archives': 'ChatArchive',
    'dm_events': 'DmEvent', 'dm_published_events': 'PublishedDmEvent',
    'event_stats': 'EventStat', 'dice_stats': 'DiceStat',
    'combat_state': 'CombatState', 'shared_canvas': 'SharedCanvas',
    'topics': 'Topic', 'announcements': 'Announcement',
    'community_posts': 'CommunityPost', 'workshop_items': 'WorkshopItem',
    'favorites': 'Favorite',
}


def _pg_export_snapshot(dst: _Path) -> dict:
    """导出 PostgreSQL 全部表数据为 JSON 快照（供备份中心）。"""
    import web.app as _app
    from sqlalchemy import inspect as _sa_inspect
    from sqlalchemy import text as _sa_text
    snapshot = {}
    with _app.db.engine.connect() as conn:
        insp = _sa_inspect(_app.db.engine)
        for table in _PG_RESTORE_ORDER:
            cols = [c['name'] for c in insp.get_columns(table)]
            rows = conn.execute(_sa_text(f'SELECT * FROM {table}')).fetchall()
            snapshot[table] = [dict(zip(cols, r)) for r in rows]
    dst.write_text(_json.dumps(snapshot, ensure_ascii=False, default=str), encoding='utf-8')
    return snapshot


def _pg_import_snapshot(src: _Path) -> list:
    """从 JSON 快照恢复 PostgreSQL（清空 + 按依赖顺序逐行插入）。"""
    import web.app as _app
    import core.models as _models
    snapshot = _json.loads(src.read_text(encoding='utf-8'))
    restored = []
    with _app.db.engine.begin() as conn:
        # 先清空全部（子表在前）
        for table in reversed(_PG_RESTORE_ORDER):
            model = getattr(_models, _MODEL_BY_TABLE.get(table, ''), None)
        ...
        for table in _PG_RESTORE_ORDER:
            ...
            if model is None:
                continue
            for row in rows:
                conn.execute(model.__table__.insert().values(**row))
...
def api_admin_create_backup():
    ...
    # PostgreSQL 全库快照导出（2026-08-15 起主数据在 PG）
    try:
        _pg_export_snapshot(backup_dir / 'dicebot_pg_snapshot.json')
        created.append('postgresql 全库快照')
    except Exception as e:
        failed.append(f'postgresql 快照: {e}')
...
def api_admin_restore_backup(name):
    ...
    for item in backup_dir.iterdir():
        ...
        try:
            if item.name == 'dicebot_pg_snapshot.json':
                # PostgreSQL 全库快照恢复
                restored.extend(_pg_import_snapshot(item))
            elif item.is_dir():
                if target.exists():
            ...
            failed.append(f'{target_rel or item.name}: {e}')

    # 数据被覆盖后，清空 SQLAlchemy 会话缓存，避免读到旧数据
    try:
        db.session.remove()
    except Exception:
        pass
