...
(function () {
    'use strict';
                ...
                // 存入资金到仓库
                window.storageDepositCoins = async function() {
                    if (!window.NORTH_CAMP_CTX.activeChar || !window.NORTH_CAMP_CTX.activeChar.coins) { alert('请先选择角色'); return; }
                    var gp = parseInt(await showPromptDialog('存入金币 (GP):', {defaultValue: window.NORTH_CAMP_CTX.activeChar.coins.gp || 0})) || 0;
                    var sp = parseInt(await showPromptDialog('存入银币 (SP):', {defaultValue: window.NORTH_CAMP_CTX.activeChar.coins.sp || 0})) || 0;
                    var cp = parseInt(await showPromptDialog('存入铜币 (CP):', {defaultValue: window.NORTH_CAMP_CTX.activeChar.coins.cp || 0})) || 0;
                ...
                // 从仓库取出资金
                window.storageWithdrawCoins = async function() {
                    if (!window.NORTH_CAMP_CTX.activeChar || !window.NORTH_CAMP_CTX.activeChar.coins) { alert('请先选择角色'); return; }
                    var gp = parseInt(await showPromptDialog('取出金币 (GP):', {defaultValue: Math.min(window.NORTH_CAMP_CTX.campStorageCoins.gp, 100)})) || 0;
                    var sp = parseInt(await showPromptDialog('取出银币 (SP):', {defaultValue: Math.min(window.NORTH_CAMP_CTX.campStorageCoins.sp, 100)})) || 0;
                    var cp = parseInt(await showPromptDialog('取出铜币 (CP):', {defaultValue: Math.min(window.NORTH_CAMP_CTX.campStorageCoins.cp, 100)})) || 0;
                    if (gp < 0 || sp < 0 || cp < 0) { alert('不能为负数'); return; }
                    if (gp === 0 && sp === 0 && cp === 0) return;
