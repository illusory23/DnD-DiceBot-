        ...
        // 状态播报（系统消息，无播报人）：角色名 进入 状态名 状态 x 回合
        function mapChatNotifyStatus(token, statusName, unit, value) {
            const tgt = (token && token.name) || '角色';
            const dur = (unit && value > 0) ? ' ' + value + ' ' + unit : '';
            const text = tgt + ' 进入 ' + statusName + ' 状态' + dur;
            fetch('/api/chat/send', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({name: '系统', text: text, color: '#ffd700', role: 'PL'})
            }).catch(function(){});
        }
        ...
        window._renderChatMsg = function(msg) {
            ...
            // 系统消息（如状态播报）不显示发送者与冒号
            var isSys = (escapedName === '系统');
            div.innerHTML = '<span class=\"cm-time\">[' + (msg.time||'') + ']</span>'
                + (isSys ? '' : '<span class=\"cm-name\" style=\"color:' + nameColor + ';\">' + escapedName + roleBadge + '</span>：')
        ...
        function collectState() {
            return {
                ...
                mapTokens: mapTokens.map(t => ({id:t.id, charId:t.charId, name:t.name, portraitUrl:t.portraitUrl, x:t.x, y:t.y, size:t.size, rotation:t.rotation||0, owner:t.owner||'', dead: !!t.dead, statuses: t.statuses || []})),
        ...
        function handleWheel(e) {
            ...
            if (target.closest('#token-info-panel') || target.closest('#layer-panel') ||
                target.closest('#status-pool-panel') || target.closest('#status-popup') ||
                target.closest('#time-panel') ||
        ...
        mapArea.addEventListener('mousedown', (e) => {
            ...
            // 点击面板时不处理地图事件（含状态池面板、状态弹窗、模组时间面板）
            if (e.target.closest('#token-info-panel') || e.target.closest('#layer-panel') ||
                e.target.closest('#status-pool-panel') || e.target.closest('#status-popup') ||
                e.target.closest('#time-panel')) return;
            // 旋转手柄
        ...
        function showTokenInfoPanel(token) {
            window._tipTokenId = token ? token.id : null;
            window._tipToken = token || null;
            ...
            tipBody.innerHTML = `
                ...
                ${(() => {
                    const sts = token.statuses || [];
                    return `<div style="text-align:center;margin-bottom:0.3rem;">
                        <button onclick="showStatusPopup(window._tipToken)" style="background:var(--surface2);border:1px solid var(--border);color:var(--text);border-radius:6px;padding:2px 10px;font-size:0.72rem;cursor:pointer;">
                            📌 状态（${sts.length ? '有·' + sts.length : '无'}）
                        </button>
                    </div>`;
                })()}
        ...
        // ━━━ 地图标记（角色头像）━━
        function createMapTokenElement(id, charId, name, portraitUrl, x, y, size, rotation, owner, dead, statuses) {
            ...
            const token = { id, charId, name, portraitUrl, x, y, size, rotation: rotation || 0, selected: false, owner: _tokOwner, dead: !!dead, statuses: Array.isArray(statuses) ? statuses : [] };
            const el = document.createElement('div');
        ...
        function tokenNetData(t) {
            return {id:t.id, charId:t.charId, name:t.name, portraitUrl:t.portraitUrl,
                    ...
                    dead: !!t.dead, statuses: t.statuses || []};
        }
        ...
        function renderAttackSourceList() {
            ...
            if (attackPickerTab === 'weapon') {
                ...
                // 仅列出可攻击法术（治疗/临时生命类在💚治疗中选择）
                const spells = (attackPickerChar.prepared_spells || []).filter(isAttackSpell);
                if (!spells.length) {
                    list.innerHTML = '<div class="attack-source-empty">该角色没有可攻击的法术（治疗法术请在💚治疗中选择）</div>';
                    return;
                }
        ...
        // 攻击法术判定：治疗/临时生命类法术（效果库明确或名称关键词识别）不进攻击列表
        function isAttackSpell(spell) {
            if (!spell || !spell.spell_name) return false;
            const eff = (typeof window.SPELL_EFFECTS !== 'undefined') ? window.SPELL_EFFECTS.get(spell.spell_name) : null;
            if (eff && (eff.kind === 'heal' || eff.kind === 'temp_hp')) return false;
            if (isHealSpell(spell)) return false;
            return true;
        }
        ...
        function switchSidebarTab(tab) {
            ...
            if (tab === 'notes' && window.loadNotes) window.loadNotes();   // loadNotes 在内层 IIFE，经 window 桥接
        }
        ...
        function mapNextCombatant() {
            ...
            if (hasLate && (nextIdx === currentIdx || (currentIdx >= 0 && nextIdx < currentIdx))) {
                ...
                if (window.tickStatusRounds) window.tickStatusRounds();   // 状态回合递减（战斗内角色，经 window 桥接）
                mapRenderInitiative();
                saveMapCombatLocal();
                debouncedSave();
                // 死亡豁免：新回合首位行动者倒地则触发
        ...
        makeDraggable(document.getElementById('status-pool-panel'), document.getElementById('status-pool-drag'));
        makeDraggable(document.getElementById('status-popup'), document.getElementById('status-popup-drag'));
        makeDraggable(document.getElementById('time-panel'), document.getElementById('time-panel-drag'));
        ...
        (async function boot() {
            ...
            // ═══ 状态系统（v5.12）：DM 状态池 + token 状态 + 回合递减 ═══
            // 状态数据挂在 token.statuses：[{id, name, addedBy, unit, value, remaining, ts}]
            // unit: '回合'|'分钟'|'小时'|null（null=永久）；value=初始时长，remaining=剩余
            // 仅 unit='回合' 随战斗轮自动递减；分钟/小时为静态时长（DM 手动删除）
            // 随 WS op tokens 同步 + 地图存档持久化

            // 状态时长格式化（弹窗完整显示）
            function fmtStatusDuration(s) {
                if (!s || !s.unit) return '永久';
                const rem = (s.remaining != null ? s.remaining : s.value);
                if (s.unit === '回合') return '剩 ' + rem + ' 回合 / 共 ' + (s.value != null ? s.value : rem) + ' 回合';
                return '剩 ' + rem + ' ' + s.unit;
            }
            // 状态时长短显示（chip：如 "中毒 2回合" / "虚弱 30分钟" / "∞"）
            function fmtStatusShort(s) {
                if (!s || !s.unit) return ' ∞';
                return ' ' + (s.remaining != null ? s.remaining : s.value) + s.unit;
            }
            // 旧数据兼容：rounds/maxRounds（v5.12 早期）→ 迁移为 unit='回合'
            function normalizeStatus(s) {
                if (!s) return s;
                if (s.unit == null && s.rounds != null) {
                    return { id: s.id, name: s.name, addedBy: s.addedBy || '', unit: '回合', value: s.maxRounds || s.rounds, remaining: s.rounds, ts: s.ts };
                }
                return s;
            }

            // token 头顶 3 秒浮动文本（"特殊状态：内容"，淡化消失）
            function showTokenFloatText(token, text) {
                if (!token || !token.el) return;
                let el = token.el.querySelector('.token-float-text');
                if (!el) {
                    el = document.createElement('div');
                    el.className = 'token-float-text';
                    token.el.appendChild(el);
                }
                el.textContent = text;
                el.style.opacity = '1';
                clearTimeout(el._floatTimer);
                el._floatTimer = setTimeout(function() {
                    el.style.opacity = '0';
                    setTimeout(function() { if (el.parentNode) el.parentNode.removeChild(el); }, 1100);
                }, 3000);
            }

            function getTokenById(id) { return mapTokens.find(function(t) { return t.id === id; }) || null; }

            // DM 添加状态（unit: '回合'|'分钟'|'小时'|null=永久；value>0 才记时长）
            function addTokenStatus(token, name, unit, value) {
                if (!token || !name) return;
                token.statuses = token.statuses || [];
                const hasDur = (unit && value > 0);
                token.statuses.push({
                    id: 's' + Date.now() + '_' + Math.floor(Math.random() * 1000),
                    name: name,
                    addedBy: (window._isDM === true) ? 'DM' : '',
                    unit: hasDur ? unit : null,
                    value: hasDur ? value : null,
                    remaining: hasDur ? value : null,
                    ts: Date.now()
                });
                syncTokenStatuses(token);
                showTokenFloatText(token, '特殊状态：' + name);   // 头顶提示（本地+WS 同步后全端）
                mapChatNotifyStatus(token, name, unit, value);   // 聊天室播报（无播报人）
            }

            function removeTokenStatus(token, statusId) {
                if (!token) return;
                token.statuses = (token.statuses || []).filter(function(s) { return s.id !== statusId; });
                syncTokenStatuses(token);
            }

            // 状态变更 → WS 广播 + 本地存档标记
            function syncTokenStatuses(token) {
                try { window._wsSendOp('tokens', [tokenNetData(token)], []); } catch(e) {}
                window._markDirty('tokens');
                // 正在显示的状态弹窗/信息面板刷新
                if (window._statusPopupFor === token.id && typeof renderStatusPopup === 'function') renderStatusPopup(token);
                if (window._tipTokenId === token.id && typeof showTokenInfoPanel === 'function') showTokenInfoPanel(token);
            }

            // 新回合：战斗内角色状态回合数 -1，归零自动移除 + 头顶提示
            window.tickStatusRounds = tickStatusRounds;   // 挂 window：mapNextRound/mapNextCombatant（外层 IIFE）经此调用
            function tickStatusRounds() {
                const inCombatIds = new Set(mapCombatants.map(function(c) { return c.charId; }).filter(Boolean));
                let changed = false;
                mapTokens.forEach(function(t) {
                    if (!t.charId || !inCombatIds.has(t.charId)) return;
                    if (!t.statuses || !t.statuses.length) return;
                    t.statuses = t.statuses.filter(function(s) {
                        if (s.unit !== '回合') return true;   // 永久/分钟/小时不随战斗轮递减
                        s.remaining = (s.remaining != null ? s.remaining : s.value) - 1;
                        if (s.remaining <= 0) {
                            changed = true;
                            showTokenFloatText(t, '状态结束：' + (s.name || ''));
                            return false;
                        }
                        changed = true;   // 递减但未移除也需同步（剩余回合变化可见）
                        return true;
                    });
                });
                if (changed) {
                    mapTokens.forEach(function(t) {
                        if (t.charId && inCombatIds.has(t.charId) && t.statuses && t.statuses.length) {
                            try { window._wsSendOp('tokens', [tokenNetData(t)], []); } catch(e) {}
                            window._markDirty('tokens');
                        }
                    });
                    if (window._statusPopupFor) { const tp = getTokenById(window._statusPopupFor); if (tp && typeof renderStatusPopup === 'function') renderStatusPopup(tp); }
                    if (window._tipTokenId) { const tp = getTokenById(window._tipTokenId); if (tp && typeof showTokenInfoPanel === 'function') showTokenInfoPanel(tp); }
                }
            }

            // ━━ DM 状态池面板（独立可拖动）━━
            window.toggleStatusPool = function() {
                const p = document.getElementById('status-pool-panel');
                if (!p) return;
                const show = p.style.display === 'none' || !p.style.display;
                p.style.display = show ? 'block' : 'none';
                if (show) renderStatusPool();
            };

            async function renderStatusPool() {
                const list = document.getElementById('status-pool-list');
                if (!list) return;
                const dm = (window._isDM === true);
                let chars = [];
                try {
                    const id = getIdentity();
                    const resp = await fetch('/api/characters?name=' + encodeURIComponent(id.name) + '&role=' + encodeURIComponent(id.role));
                    const data = await resp.json();
                    chars = Array.isArray(data) ? data : (data.characters || []);
                } catch(e) {}
                const rows = [];
                const seen = new Set();
                mapTokens.forEach(function(t) {
                    const key = t.charId || ('n:' + t.name);
                    seen.add(key);
                    rows.push({ label: (t.charId ? '' : '🧷 ') + (t.name || '未命名'), token: t, inMap: true });
                });
                chars.forEach(function(c) {
                    if (!c || !c.id || seen.has(c.id)) return;
                    rows.push({ label: (c.name || '') + '（未拖入地图）', token: null, inMap: false });
                });
                // 未拖入地图的角色折叠区（可展开）
                const offMap = rows.filter(function(r) { return !r.inMap; });
                const offHtml = offMap.length
                    ? '<details class="st-collapse"><summary>⚡ 未拖入地图的角色（' + offMap.length + '）</summary>'
                        + offMap.map(function(r) { return statusPoolRow(r, dm); }).join('') + '</details>'
                    : '';
                const onMap = rows.filter(function(r) { return r.inMap; });
                list.innerHTML = onMap.map(function(r) { return statusPoolRow(r, dm); }).join('') + offHtml;
                renderQuickStatuses();
            }

            function statusPoolRow(r, dm) {
                const sts = r.token ? (r.token.statuses || []) : [];
                const chips = sts.map(function(s) {
                    const dur = (s && s.unit)
                        ? ` <span class="st-rounds" onclick="editStatusDuration(${r.token.id}, '${s.id}')" title="点击编辑时长">${fmtStatusShort(s)}</span>`
                        : ` <span class="st-forever" onclick="editStatusDuration(${r.token.id}, '${s.id}')" title="点击编辑时长">∞</span>`;
                    const del = dm ? ` <span class="st-del" onclick="delTokenStatus(${r.token.id}, '${s.id}')" title="删除">×</span>` : '';
                    return `<span class="status-chip" data-sid="${s.id}">` + escapeHtml(s.name) + dur + del + '</span>';
                }).join('');
                const addBtn = (dm && r.inMap)
                    ? '<span style="display:flex;gap:3px;margin-top:4px;flex-wrap:wrap;">'
                        + '<input class="st-input" id="st-name-' + r.token.id + '" placeholder="状态名" style="flex:1;min-width:56px;">'
                        + '<select class="st-input" id="st-unit-' + r.token.id + '" style="width:54px;" title="单位（回合随战斗轮递减；分钟/小时为静态时长）">'
                        + '<option value="回合">回合</option><option value="分钟">分钟</option><option value="小时">小时</option><option value="">永久</option>'
                        + '</select>'
                        + '<input class="st-input" id="st-value-' + r.token.id + '" type="number" min="1" placeholder="时长" style="width:46px;" title="时长数值（回合/分钟/小时）">'
                        + '<button class="st-add" onclick="doAddTokenStatus(' + r.token.id + ')">➕</button>'
                      + '</span>'
                    : '';
                const hint = r.inMap ? '' : '<div style="font-size:0.68rem;color:var(--text-dim);">将角色拖入地图后可添加状态</div>';
                // 行头：角色名 + 一键清空（仅 DM、有状态时）
                const clearBtn = (dm && r.inMap && sts.length)
                    ? '<button class="st-clear" onclick="clearTokenStatuses(' + r.token.id + ')" title="一键清空全部状态">🗑 清空</button>'
                    : '';
                return '<div class="status-pool-row" ondragover="quickDragOver(event)" ondragleave="quickDragLeave(event)" ondrop="quickDrop(event, ' + (r.token ? r.token.id : -1) + ')">'
                    + '<div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;">👤 <b>' + escapeHtml(r.label) + '</b> ' + clearBtn + '</div>'
                    + '<div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;margin-top:2px;">' + chips + '</div>'
                    + addBtn + hint
                    + '</div>';
            }

            window.doAddTokenStatus = function(tokenId) {
                const token = getTokenById(tokenId);
                if (!token) return;
                const nameEl = document.getElementById('st-name-' + tokenId);
                const unitEl = document.getElementById('st-unit-' + tokenId);
                const valEl = document.getElementById('st-value-' + tokenId);
                const name = (nameEl && nameEl.value.trim()) || '';
                if (!name) return;
                const unit = (unitEl && unitEl.value) || '';
                const value = valEl ? parseInt(valEl.value, 10) : 0;
                addTokenStatus(token, name, unit, value);
                if (nameEl) nameEl.value = '';
                if (valEl) valEl.value = '';
                renderStatusPool();
            };

            window.delTokenStatus = function(tokenId, statusId) {
                const token = getTokenById(tokenId);
                if (!token) return;
                removeTokenStatus(token, statusId);
                renderStatusPool();
            };

            // 一键清空角色全部状态（DM）
            window.clearTokenStatuses = async function(tokenId) {
                const token = getTokenById(tokenId);
                if (!token) return;
                const n = (token.statuses || []).length;
                if (!n) return;
                const msg = '清空 ' + (token.name || '该角色') + ' 的全部状态（' + n + ' 个）？';
                if (typeof showConfirmDialog === 'function') {
                    const okc = await showConfirmDialog(msg);
                    if (!okc) return;
            ...
            // 编辑状态时长：chip 时长点击 → 内联编辑（单位 + 数值 + ✓）
            window.editStatusDuration = function(tokenId, statusId) {
                const chip = document.querySelector('.status-chip[data-sid="' + statusId + '"]');
                if (!chip) return;
                const token = getTokenById(tokenId);
                const s = token ? (token.statuses || []).find(function(x) { return x.id === statusId; }) : null;
                if (!s) return;
                chip.innerHTML = `<select class="st-input" id="ed-unit-${statusId}" style="width:52px;">`
                    + `<option value="回合"${s.unit === '回合' ? ' selected' : ''}>回合</option>`
                    + `<option value="分钟"${s.unit === '分钟' ? ' selected' : ''}>分钟</option>`
                    + `<option value="小时"${s.unit === '小时' ? ' selected' : ''}>小时</option>`
                    + `<option value=""${!s.unit ? ' selected' : ''}>永久</option></select>`
                    + `<input class="st-input" id="ed-val-${statusId}" type="number" min="1" value="${s.value != null ? s.value : 1}" style="width:40px;">`
                    + `<span class="st-add" onclick="saveStatusDuration(${tokenId}, '${statusId}')" style="padding:1px 6px;">✓</span>`;
            };

            // 保存编辑后的时长
            window.saveStatusDuration = function(tokenId, statusId) {
                const token = getTokenById(tokenId);
                if (!token) return;
                const s = (token.statuses || []).find(function(x) { return x.id === statusId; });
                if (!s) return;
                const unitEl = document.getElementById('ed-unit-' + statusId);
                const valEl = document.getElementById('ed-val-' + statusId);
                const unit = unitEl ? unitEl.value : '';
                const value = valEl ? Math.max(1, parseInt(valEl.value, 10) || 1) : 1;
                s.unit = unit || null;
                s.value = unit ? value : null;
                s.remaining = unit ? value : null;
                syncTokenStatuses(token);
                renderStatusPool();
            };

            // ━━ 快捷状态词条（localStorage 持久化，拖拽到角色行）━━
            const QUICK_KEY = 'map_quick_statuses';
            const DEFAULT_QUICK = ['中毒', '流血', '束缚', '目盲', '倒地', '昏迷', '麻痹', '石化', '魅惑', '恐惧', '虚弱', '眩晕', '耳聋', '受惊', '隐形', '纠缠', '失明', '中毒深重'];
            function loadQuickStatuses() {
                try { const v = localStorage.getItem(QUICK_KEY); if (v) { const arr = JSON.parse(v); if (Array.isArray(arr)) return arr; } } catch(e) {}
                return DEFAULT_QUICK.slice();
            }
            function saveQuickStatuses(list) {
                try { localStorage.setItem(QUICK_KEY, JSON.stringify(list)); } catch(e) {}
            }
            window.addQuickStatus = function() {
                const el = document.getElementById('st-quick-new');
                const name = (el && el.value.trim()) || '';
                if (!name) return;
                const list = loadQuickStatuses();
                if (!list.includes(name)) list.push(name);
                saveQuickStatuses(list);
                if (el) el.value = '';
                renderQuickStatuses();
            };
            window.delQuickStatus = function(name) {
                const list = loadQuickStatuses().filter(function(n) { return n !== name; });
                saveQuickStatuses(list);
                renderQuickStatuses();
            };
            function renderQuickStatuses() {
                const el = document.getElementById('status-quick-list');
                if (!el) return;
                el.innerHTML = loadQuickStatuses().map(function(n) {
                    const esc = String(n).replace(/'/g, "\\'");
                    return '<div class="status-quick-item" draggable="true" ondragstart="quickDragStart(event,\'' + esc + '\')">'
                        + '<span>⚡ ' + escapeHtml(n) + '</span>'
                        + '<span class="st-del" onclick="delQuickStatus(\'' + esc + '\')" title="删除词条">×</span>'
                        + '</div>';
                }).join('');
            }

            // 拖拽：快捷词条 → 角色行
            window.quickDragStart = function(ev, name) {
                ev.dataTransfer.setData('text/plain', name);
                ev.dataTransfer.effectAllowed = 'copy';
            };
            window.quickDragOver = function(ev) {
                ev.preventDefault();
                ev.dataTransfer.dropEffect = 'copy';
                const row = ev.currentTarget;
                if (row && !row.classList.contains('drag-over')) row.classList.add('drag-over');
            };
            window.quickDragLeave = function(ev) {
                const row = ev.currentTarget;
            ...
            window.quickDrop = function(ev, tokenId) {
                ...
                const row = ev.currentTarget;
                if (row) row.classList.remove('drag-over');
                if (tokenId < 0) return;   // 未拖入地图的角色行不可投放
                const token = getTokenById(tokenId);
                if (!token) return;
                const name = ev.dataTransfer.getData('text/plain');
                if (!name) return;
                const unitEl = document.getElementById('st-quick-unit');
                const valEl = document.getElementById('st-quick-value');
                const unit = (unitEl && unitEl.value) || '';
                const value = valEl ? parseInt(valEl.value, 10) : 0;
                addTokenStatus(token, name, unit, value);
                renderStatusPool();
            ...
            // ━━ 状态详情弹窗（信息面板"📌 状态"按钮打开）━━
            window._statusPopupFor = null;
            window.showStatusPopup = function(token) {
                if (!token) return;
                window._statusPopupFor = token.id;
                const p = document.getElementById('status-popup');
                if (p) p.style.display = 'block';
                renderStatusPopup(token);
            };
            window.hideStatusPopup = function() {
                window._statusPopupFor = null;
                const p = document.getElementById('status-popup');
                if (p) p.style.display = 'none';
            };
            function renderStatusPopup(token) {
                const body = document.getElementById('status-popup-body');
                if (!body) return;
                const dm = (window._isDM === true);
                const sts = token.statuses || [];
                const items = sts.length ? sts.map(function(s) {
                    const rnd = fmtStatusDuration(s);   // 状态名/剩余时间（回合/分钟/小时/永久）
                    const del = dm ? ' <button onclick="delTokenStatus(' + token.id + ", '" + s.id + "')\" style=\"background:var(--red);border:none;color:#fff;border-radius:4px;padding:0 6px;font-size:0.65rem;cursor:pointer;\">删除</button>" : '';
                    return '<div style="padding:4px 6px;border:1px solid var(--border);border-radius:6px;margin-bottom:4px;font-size:0.75rem;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">'
                        + '<b>' + escapeHtml(s.name) + '</b>'
                        + '<span style="color:var(--text-dim);">' + rnd + (s.addedBy ? ' · ' + escapeHtml(s.addedBy) : '') + '</span>'
                        + del + '</div>';
                }).join('') : '<div style="font-size:0.75rem;color:var(--text-dim);text-align:center;padding:0.5rem;">暂无状态</div>';
                body.innerHTML = '<div style="font-size:0.8rem;margin-bottom:0.3rem;color:var(--gold);">📌 ' + escapeHtml(token.name) + '</div>' + items;
            }

            // ═══ 模组时间（v5.12）：右上竖条 + 可拖动面板，全员设置/手动推进 ═══
            let _timeCfg = { mph: 60, hpd: 24, dpm: 30, mpy: 12 };   // 历法进位缓存（自定义推进换算用）
            let _timeEmpty = true;                                    // 未设置/已清空标记
            window.toggleTimePanel = function() {
                const p = document.getElementById('time-panel');
                if (!p) return;
                const show = p.style.display === 'none' || !p.style.display;
                p.style.display = show ? 'block' : 'none';
                if (show) loadCampaignTime();
            };

            async function loadCampaignTime() {
                ...
                try {
                    ...
                    const d = await resp.json();
                    if (!d || d.error) return;
                    const meta = document.getElementById('time-meta');
                    const ctrl = document.getElementById('time-dm-controls');
                    if (ctrl) ctrl.style.display = (window._isDM === true) ? '' : 'none';   // 编辑仅 DM 可见
                    if (d.empty) {
                        // 未设置/已清空 → 空状态
                        _timeEmpty = true;
                        disp.textContent = '⏳ 未设置模组时间';
                        if (meta) meta.textContent = '请先设置起始时间';
                        ['time-era', 'time-year', 'time-month', 'time-day', 'time-hour', 'time-minute',
                         'time-mph', 'time-hpd', 'time-dpm', 'time-mpy'].forEach(function(id) {
                            const el = document.getElementById(id);
                            if (el) el.value = '';
                        });
                        return;
                    ...
                    _timeEmpty = false;
                    _timeCfg = { mph: d.minutes_per_hour || 60, hpd: d.hours_per_day || 24, dpm: d.days_per_month || 30, mpy: d.months_per_year || 12 };
                    disp.textContent = `${d.era} ${d.year} 年 ${d.month} 月 ${d.day} 日 ${d.hour} 时 ${d.minute} 分`;
                    if (meta) meta.textContent = d.updated_by ? `由 ${d.updated_by} 更新 · ${d.updated_at}` : '';
                    [['time-era', d.era], ['time-year', d.year], ['time-month', d.month], ['time-day', d.day], ['time-hour', d.hour], ['time-minute', d.minute],
                     ['time-mph', d.minutes_per_hour], ['time-hpd', d.hours_per_day], ['time-dpm', d.days_per_month], ['time-mpy', d.months_per_year]].forEach(function(pair) {
            ...
            // 推进时间（挂 window 供 HTML onclick 调用；单位换算在 advanceTimeCustom）
            window.advanceTime = async function(minutes) {
                if (window._isDM !== true) return;
                if (_timeEmpty) { if (typeof Toast !== 'undefined') Toast.info('请先设置起始时间'); return; }
                                try {
                    const s = JSON.parse(sessionStorage.getItem('dnd_joined_room') || '{}');
                    const resp = await fetch('/api/campaign-time', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({name: s.name || '', advance: minutes})});
                    const d = await resp.json();
                    if (d.error) { if (typeof Toast !== 'undefined') Toast.info(d.error); return; }
                    loadCampaignTime();
                } catch(e) {}
            };

            // 自定义推进：数值 + 单位（分钟/小时/天/月，按当前历法进位换算为分钟）
            window.advanceTimeCustom = function() {
                if (window._isDM !== true) return;
                const el = document.getElementById('time-advance-input');
                const unitEl = document.getElementById('time-advance-unit');
                const v = el ? parseInt(el.value, 10) : 0;
                if (v <= 0) return;
                const unit = unitEl ? unitEl.value : 'minute';
                let minutes = v;
                if (unit === 'hour') minutes = v * _timeCfg.mph;
                else if (unit === 'day') minutes = v * _timeCfg.mph * _timeCfg.hpd;
                else if (unit === 'month') minutes = v * _timeCfg.mph * _timeCfg.hpd * _timeCfg.dpm;
                advanceTime(minutes);
                if (el) el.value = '';
            };

            // +1月 快捷按钮（按当前历法进位换算为分钟）
            window.advanceTimeCustomMonth = function() {
                if (window._isDM !== true) return;
                advanceTime(_timeCfg.mph * _timeCfg.hpd * _timeCfg.dpm);
            };

            // 清空（重置）模组时间：恢复默认历法/时间/进位（新模组开团用，DM）
            window.resetCampaignTime = async function() {
                if (window._isDM !== true) return;
                const msg = '清空模组时间？（清空后需重新设置起始时间）';
                if (typeof showConfirmDialog === 'function') {
                    const okc = await showConfirmDialog(msg);
                    if (!okc) return;
                } else if (!confirm(msg)) return;
                try {
                    const s = JSON.parse(sessionStorage.getItem('dnd_joined_room') || '{}');
                    const resp = await fetch('/api/campaign-time', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({name: s.name || '', clear: true})});
                    const d = await resp.json();
                    if (d.error) { if (typeof Toast !== 'undefined') Toast.info(d.error); return; }
                    loadCampaignTime();
                    if (typeof Toast !== 'undefined') Toast.show('✅ 模组时间已清空', 'success');
                } catch(e) {}
            };

            // 单独保存进位配置（不随起始时间）
            window.saveTimeConfig = async function() {
                if (window._isDM !== true) return;
                                const g = function(id) { const el = document.getElementById(id); return el ? el.value : ''; };
                try {
                    const s = JSON.parse(sessionStorage.getItem('dnd_joined_room') || '{}');
                    const resp = await fetch('/api/campaign-time', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({
                        name: s.name || '', set: true,
                        minutes_per_hour: parseInt(g('time-mph'), 10) || 60,
                        hours_per_day: parseInt(g('time-hpd'), 10) || 24,
                        days_per_month: parseInt(g('time-dpm'), 10) || 30,
                        months_per_year: parseInt(g('time-mpy'), 10) || 12
                    })});
                    const d = await resp.json();
                    if (d.error) { if (typeof Toast !== 'undefined') Toast.info(d.error); return; }
                    loadCampaignTime();
                    if (typeof Toast !== 'undefined') Toast.show('✅ 进位配置已保存', 'success');
                } catch(e) {}
            };

            window.setCampaignTime = async function() {
                if (window._isDM !== true) return;
                                const g = function(id) { const el = document.getElementById(id); return el ? el.value : ''; };
                try {
                    const s = JSON.parse(sessionStorage.getItem('dnd_joined_room') || '{}');
                    const resp = await fetch('/api/campaign-time', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({
                        name: s.name || '', set: true,
                        era: g('time-era') || '圣历',
                        year: parseInt(g('time-year'), 10) || 1,
                        month: parseInt(g('time-month'), 10) || 1,
                        day: parseInt(g('time-day'), 10) || 1,
                        hour: parseInt(g('time-hour'), 10) || 0,
                        minute: parseInt(g('time-minute'), 10) || 0
                    })});
                    const d = await resp.json();
                    if (d.error) { if (typeof Toast !== 'undefined') Toast.info(d.error); return; }
                    loadCampaignTime();
            ...
            // 30 秒轮询保持时间同步（轻量）
            setInterval(loadCampaignTime, 30000);

            // ═══ 线索与委托笔记（v5.12）：侧边栏 Tab，DM/PL 可编辑，公开/私有 ═══
            let notesKind = 'clue';
            let notesCache = [];
            window.loadNotes = async function() {   // 挂 window：switchSidebarTab（外层 IIFE）经此调用
                return loadNotesInner();
            };
            async function loadNotesInner() {
                const panel = document.getElementById('sidebar-panel-notes');
                if (!panel) {
                    console.error('笔记面板元素不存在：sidebar-panel-notes');
                    return;
                }
                panel.innerHTML = '<div style="padding:0.8rem;font-size:0.75rem;color:var(--text-dim);text-align:center;">加载中...</div>';
                try {
                    const resp = await fetch('/api/notes?kind=' + notesKind);
                    if (resp.status === 401) {
                        panel.innerHTML = '<div style="padding:0.8rem;font-size:0.75rem;color:var(--gold);text-align:center;line-height:1.8;">🔒 请先登录官网账号<br><span style="color:var(--text-dim);font-size:0.68rem;">登录后笔记跨设备同步<br><a href="/" style="color:var(--accent);">前往官网登录 →</a></span></div>';
                        return;
                    }
                    const data = await resp.json();
                    ...
                    panel.innerHTML = '<div style="padding:0.8rem;font-size:0.75rem;color:var(--red);text-align:center;">加载失败：' + String(e && e.message || e) + '</div>';
                }
            }

            window.switchNotesKind = function(kind) {
                notesKind = kind;
                document.querySelectorAll('#notes-kind-tabs button').forEach(function(b) { b.classList.toggle('active', b.dataset.kind === kind); });
                loadNotesInner();
            };

            function noteStatusBadge(s) {
                if (!s) return '';
                const colors = { '未确认':'var(--gold)', '已确认':'var(--green)', '已排除':'var(--text-dim)', '未开始':'var(--text-dim)', '进行中':'var(--gold)', '已完成':'var(--green)', '失败':'var(--red)' };
                const cc = colors[s] || 'var(--text-dim)';
                return `<span style="color:${cc};border:1px solid ${cc};border-radius:8px;padding:0 5px;font-size:0.62rem;margin-left:4px;">${s}</span>`;
            }
            ...
            function renderNotes() {
                ...
                if (!panel) return;
                try {
                renderNotesInner(panel);
                } catch(e) {
                    console.error('笔记渲染失败:', e);
                    panel.innerHTML = '<div style="padding:0.8rem;font-size:0.75rem;color:var(--red);text-align:center;">渲染失败：' + String(e && e.message || e) + '</div>';
                }
            }
            function renderNotesInner(panel) {
                const isQuest = notesKind === 'quest';
                const list = notesCache.map(function(n) {
                    const vis = n.visibility === 'private' ? '🔒 私有' : '🌐 公开';
                    const meta = isQuest
                        ? ((n.source ? '委托人：' + n.source : '') + (n.reward ? ' · 奖励：' + n.reward : ''))
                        : ((n.source ? '来源：' + n.source : '') + (n.npc ? ' · NPC：' + n.npc : ''));
                    return '<div style="border:1px solid var(--border);border-radius:6px;padding:5px 7px;margin-bottom:5px;background:var(--bg);font-size:0.72rem;">'
                        + '<div style="display:flex;align-items:center;gap:3px;flex-wrap:wrap;"><b>' + escapeHtml(n.title) + '</b>' + noteStatusBadge(n.status)
                        + '<span style="font-size:0.6rem;color:var(--text-dim);margin-left:auto;">' + vis + '</span></div>'
                        + (meta ? '<div style="color:var(--text-dim);font-size:0.66rem;">' + escapeHtml(meta) + '</div>' : '')
                        + (n.description ? '<div style="color:var(--text);font-size:0.68rem;margin-top:2px;">' + escapeHtml(n.description) + '</div>' : '')
                        + '<div style="color:var(--text-dim);font-size:0.6rem;margin-top:2px;">' + escapeHtml(n.created_by || '') + ' · ' + escapeHtml(n.updated_at || '') + '</div>'
                        + '<div style="display:flex;gap:4px;margin-top:3px;">'
                        + '<button class="st-add" onclick="editNoteForm(' + n.id + ')" style="padding:0 6px;" title="编辑">✏️</button>'
                        + '<button class="st-add" onclick="deleteNote(' + n.id + ')" style="padding:0 6px;background:var(--red);border-color:var(--red);" title="删除">🗑</button>'
                        + '</div><div id="note-edit-' + n.id + '"></div></div>';
                }).join('');
                panel.innerHTML = '<div style="display:flex;align-items:center;gap:6px;padding:5px 8px;border-bottom:1px solid var(--border);flex-shrink:0;">'
                    + '<div class="sidebar-tabs" id="notes-kind-tabs" style="flex:1;border:none;">'
                    + '<button class="sidebar-tab' + (!isQuest ? ' active' : '') + '" data-kind="clue" onclick="switchNotesKind(\'clue\')">🔍 线索</button>'
                    + '<button class="sidebar-tab' + (isQuest ? ' active' : '') + '" data-kind="quest" onclick="switchNotesKind(\'quest\')">📋 委托</button>'
                    + '</div>'
                    + '<button class="st-add" onclick="newNoteForm()" title="新增">➕</button></div>'
                    + '<div style="padding:5px 8px;overflow-y:auto;flex:1;" id="notes-list">'
                    + (list || '<div style="color:var(--text-dim);font-size:0.72rem;text-align:center;padding:1rem;">暂无' + (isQuest ? '委托' : '线索') + '</div>')
                    + '</div>'
                    + '<div id="note-form-area" style="padding:6px 8px;border-top:1px solid var(--border);flex-shrink:0;"></div>';
            }

            function noteFormHtml(n) {
                const isQuest = notesKind === 'quest';
                const statuses = isQuest ? ['未开始', '进行中', '已完成', '失败'] : ['未确认', '已确认', '已排除'];
                const statusOpts = statuses.map(function(s) { return '<option' + (n && n.status === s ? ' selected' : '') + '>' + s + '</option>'; }).join('');
                return '<div style="display:flex;flex-direction:column;gap:4px;font-size:0.72rem;">'
                    + '<input class="st-input" id="nf-title" placeholder="标题 *" value="' + (n ? escapeHtml(n.title) : '') + '" style="width:100%;">'
                    + '<textarea class="st-input" id="nf-desc" placeholder="描述" style="width:100%;min-height:46px;">' + (n ? escapeHtml(n.description || '') : '') + '</textarea>'
                    + '<div style="display:flex;gap:4px;flex-wrap:wrap;">'
                    + '<input class="st-input" id="nf-source" placeholder="' + (isQuest ? '委托人' : '来源') + '" value="' + (n ? escapeHtml(n.source || '') : '') + '" style="flex:1;min-width:64px;">'
                    + (isQuest
                        ? '<input class="st-input" id="nf-reward" placeholder="奖励" value="' + (n ? escapeHtml(n.reward || '') : '') + '" style="flex:1;min-width:64px;">'
                        : '<input class="st-input" id="nf-npc" placeholder="关联NPC" value="' + (n ? escapeHtml(n.npc || '') : '') + '" style="flex:1;min-width:64px;">')
                    + '</div>'
                    + '<div style="display:flex;gap:4px;align-items:center;flex-wrap:wrap;">'
                    + '<select class="st-input" id="nf-status" style="width:72px;">' + statusOpts + '</select>'
                    + '<select class="st-input" id="nf-vis" style="width:76px;">'
                    + '<option value="public"' + (!n || n.visibility !== 'private' ? ' selected' : '') + '>🌐 公开</option>'
                    + '<option value="private"' + (n && n.visibility === 'private' ? ' selected' : '') + '>🔒 私有</option>'
                    + '</select>'
                    + '<button class="st-add" onclick="saveNote(' + (n ? n.id : 0) + ')">保存</button>'
                    + '<button class="st-add" onclick="cancelNoteForm()" style="background:var(--surface2);border-color:var(--border);color:var(--text);">取消</button>'
                    + '</div></div>';
            }

            window.newNoteForm = function() { const a = document.getElementById('note-form-area'); if (a) a.innerHTML = noteFormHtml(null); };
            window.editNoteForm = function(id) {
                const n = notesCache.find(function(x) { return x.id === id; });
                if (!n) return;
                const box = document.getElementById('note-edit-' + id);
                if (box) box.innerHTML = noteFormHtml(n);
            };
            window.cancelNoteForm = function() { const a = document.getElementById('note-form-area'); if (a) a.innerHTML = ''; };
            window.saveNote = async function(id) {
                const g = function(eid) { const el = document.getElementById(eid); return el ? el.value : ''; };
                const body = {
                    kind: notesKind,
                    title: g('nf-title').trim(),
                    description: g('nf-desc'),
                    source: g('nf-source'),
                    npc: g('nf-npc'),
                    reward: g('nf-reward'),
                    status: g('nf-status'),
                    visibility: g('nf-vis') || 'public',
                };
                if (!body.title) { if (typeof Toast !== 'undefined') Toast.info('标题不能为空'); return; }
                try {
                    const resp = await fetch(id ? '/api/notes/' + id : '/api/notes', {method: id ? 'PUT' : 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(body)});
                    const d = await resp.json();
                    if (d.error) { if (typeof Toast !== 'undefined') Toast.info(d.error); return; }
                    const a = document.getElementById('note-form-area');
                    if (a) a.innerHTML = '';
                    loadNotesInner();
                } catch(e) {}
            };
            window.deleteNote = async function(id) {
                if (typeof showConfirmDialog === 'function') {
                    const okc = await showConfirmDialog('删除这条' + (notesKind === 'quest' ? '委托' : '线索') + '？');
                    if (!okc) return;
                } else if (!confirm('删除这条记录？')) return;
                try { await fetch('/api/notes/' + id, {method: 'DELETE'}); loadNotesInner(); } catch(e) {}
            };
            ...
            function applyRoleRestrictions() {
                ...
                var statusPoolBtn = document.getElementById('status-pool-btn');
                var statusPoolPanel = document.getElementById('status-pool-panel');
                ...
                if (dm) {
                    ...
                    if (statusPoolBtn) statusPoolBtn.style.display = '';
                } else {
                    ...
                    if (statusPoolBtn) statusPoolBtn.style.display = 'none';
                    if (statusPoolPanel) statusPoolPanel.style.display = 'none';
                }
            ...
            function mergeRemoteTokens(list, removeIds, removeMissing) {
                ...
                mapTokens = mapTokens.filter(function(t) {
                    ...
                    if (td) {
                        ...
                        // 状态同步：新状态（按 id 对比）→ 头顶 3 秒提示"特殊状态：内容"（全体可见）
                        if (Array.isArray(td.statuses)) {
                            const oldIds = new Set((t.statuses || []).map(s => s.id));
                            const fresh = td.statuses.filter(s => s && !oldIds.has(s.id));
                            t.statuses = td.statuses.map(normalizeStatus);   // 旧数据（rounds 字段）迁移
                            if (fresh.length && t.el) {
                                fresh.forEach(function(s) {
                                    if (typeof showTokenFloatText === 'function') {
                                        showTokenFloatText(t, '特殊状态：' + (s.name || ''));
                                    }
                                });
                            }
                            // 状态面板若正显示该 token，刷新
                            if (window._statusPopupFor === t.id && typeof renderStatusPopup === 'function') {
                                renderStatusPopup(t);
                            }
                        }
                        if (t.el) {
                ...
                Object.keys(incoming).forEach(function(id) {
                    ...
                    mapTokens.push(createMapTokenElement(td.id, td.charId, td.name, td.portraitUrl, td.x, td.y, td.size||48, td.rotation||0, td.owner, td.dead, td.statuses));
