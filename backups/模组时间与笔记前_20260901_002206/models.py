...
class ContentView(db.Model):
    """内容浏览量（v5.8）：每用户每内容仅计一次（去重）。
    浏览量 = 本表计数（content_type + content_id 的记录数）。
    """
    __tablename__ = 'content_views'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    content_type = db.Column(db.String(10), nullable=False)   # post | workshop
    content_id = db.Column(db.Integer, nullable=False)
    user_id = db.Column(db.Integer, nullable=False)
    created_at = db.Column(db.String(20), default='')
    __table_args__ = (db.UniqueConstraint('content_type', 'content_id', 'user_id',
                                          name='uq_content_view'),)
...
class CampaignTime(db.Model):
    """模组时间（跑团时历，全团共享单行，2026-09-01 新增）

    由 DM 设置起始时间并手动推进（+分钟），全体玩家只读展示。
    """
    __tablename__ = 'campaign_time'

    id = db.Column(db.Integer, primary_key=True)
    era = db.Column(db.String(20), default='圣历')    # 历法名（如 圣历/龙历）
    ...
    # 历法进位配置（不同模组历法不同，可编辑；默认与常用历一致）
    minutes_per_hour = db.Column(db.Integer, default=60)   # 1 小时 = N 分钟
    hours_per_day = db.Column(db.Integer, default=24)      # 1 日   = N 小时
    days_per_month = db.Column(db.Integer, default=30)     # 1 月   = N 日
    months_per_year = db.Column(db.Integer, default=12)    # 1 年   = N 月
    updated_by = db.Column(db.String(50), default='')
    updated_at = db.Column(db.String(20), default='')


class Note(db.Model):
    """线索与委托笔记（DM/PL 均可编辑，2026-09-01 新增）

    kind: clue=线索 / quest=委托
    status: 线索=未确认/已确认/已排除；委托=未开始/进行中/已完成/失败
    visibility: public=公开（全团可见可编辑）/ private=私有（仅创建者可见）
    """
    __tablename__ = 'notes'

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    kind = db.Column(db.String(10), nullable=False, default='clue')
    title = db.Column(db.String(100), nullable=False)
    description = db.Column(db.Text, default='')
    source = db.Column(db.String(100), default='')    # 线索来源 / 委托人
    npc = db.Column(db.String(100), default='')       # 关联 NPC（线索）
    reward = db.Column(db.String(100), default='')    # 奖励（委托）
    status = db.Column(db.String(20), default='')
    visibility = db.Column(db.String(10), default='public')   # public/private
    owner_id = db.Column(db.Integer, default=None)             # 私有归属（公开时为空）
