...
def _ensure_campaign_time_columns():
    """旧库补列：campaign_time 历法进位配置（幂等，2026-09-01）"""
    try:
        from sqlalchemy import inspect as _sa_inspect
        cols = {c['name'] for c in _sa_inspect(db.engine).get_columns('campaign_time')}
        with db.engine.begin() as conn:
            if 'minutes_per_hour' not in cols:
                conn.exec_driver_sql("ALTER TABLE campaign_time ADD COLUMN minutes_per_hour INTEGER NOT NULL DEFAULT 60")
            if 'hours_per_day' not in cols:
                conn.exec_driver_sql("ALTER TABLE campaign_time ADD COLUMN hours_per_day INTEGER NOT NULL DEFAULT 24")
            if 'days_per_month' not in cols:
                conn.exec_driver_sql("ALTER TABLE campaign_time ADD COLUMN days_per_month INTEGER NOT NULL DEFAULT 30")
            if 'months_per_year' not in cols:
                conn.exec_driver_sql("ALTER TABLE campaign_time ADD COLUMN months_per_year INTEGER NOT NULL DEFAULT 12")
    except Exception as _e:
        _app_logger.warning(f'补列 campaign_time 表失败: {_e}')
...
# 心跳超时秒数：挂机玩家页面仍打开（浏览器后台节流/网络抖动）时不误清。
# 原硬编码 10 秒与心跳间隔 10 秒相等，任何一次延迟都会被当作离线清理掉。
ROOM_HEARTBEAT_TIMEOUT = 30
...
def dice_help_page():
    """掷骰帮助页（指令说明 + 本地独立演示掷骰）"""
    return render_template('dice-help.html')
...
def api_roll():
    ...
    try:
        ...
        return jsonify({
            ...
            'groups_detail': result.groups_detail,
            'char_name': char_name,
...
def api_adjust_hp(name):
    ...
    if data.get('temp'):
        # 临时生命：设置临时 HP（>=0，无上限；法术效果库 temp_hp 类法术使用）
        from core.character import set_hp, get_character
        ok_set = set_hp(char['id'], temp_hp=int(amount))
        ch = get_character(char['id'])
        return jsonify({'ok': bool(ok_set), 'temp_hp': ch['temp_hp'] if ch else int(amount)})
...
# 怪物数据规则版本标注（仅标注展示，不修正/覆盖/删除任何数据）
# 5r = 2025 新版规则（怪物图鉴2025）；5e = 2014 规则（LMoP 附录B / SRD）
_RULE_VERSION_MAP = {
    'MM25': '5r 规则（怪物图鉴2025）',
    'MM': '5e 规则（怪物图鉴2014）',
    'FR': '5r 规则（被遗忘的国度）',
...
def _rule_version_of(source):
    if not source:
        return ''
    if source in _RULE_VERSION_MAP:
        return _RULE_VERSION_MAP[source]
    if '凡度尔' in str(source) or '失落矿坑' in str(source):
        return '5e 规则（LMoP 附录B）'
    if source == '自定义':
        return '5e 规则（自定义）'
    return source
...
def api_search_monsters():
    ...
    # 去重键含 source：同名不同规则版本（5e MM / 5r MM25）都展示
    seen_keys = set()
    ...
    def _make_item(name, name_en='', cr='', size='', mtype='', source='', detail='', detail_text='', detail_link=''):
        return {
            ...
            'rule_version': _rule_version_of(source),
    ...
    try:
        ...
        for m in _load_srd():
            name = m.get('name', '')
            if (name, 'SRD') in seen_keys: continue
            score = _calc_match_score(query, name, m.get('name_en', ''))
            if score > 0:
                seen_keys.add((name, 'SRD'))
                scored_items.append((score, _make_item(
                    name, m.get('name_en', ''), m.get('cr', ''), m.get('size', ''),
                    m.get('type', ''), 'SRD',
                )))
...
def _monster_exact_hit(monster: dict, q_lower: str) -> bool:
    """怪物条目是否与查询名精确一致（中文名/混合名/英文名任一精确相等）"""
    return any(
        str(monster.get(f) or '').strip().lower() == q_lower
        for f in ('name_cn', 'name', 'name_en')
    )


def _srd_monster_exact(q_lower: str) -> dict | None:
    """SRD 怪物精确匹配（name / name_en 精确相等），精确优先于包含匹配"""
    from utils.data_loader import load_monsters
    for m in load_monsters():
        if str(m.get('name') or '').strip().lower() == q_lower:
            return m
        if str(m.get('name_en') or '').strip().lower() == q_lower:
            return m
    return None
...
def api_search_monster(name):
    ...
    匹配分两轮：先在所有数据源中找「精确同名」条目（避免点击列表中的
    "地精"却拿到名称包含"地精"的"熊地精"这类错配），全部精确失败后
    才用模糊包含匹配兜底（自定义最短 → CHM 评分第一 → SRD 首个包含）。

    支持 ?source= 指定规则版本（如 source=MM 只查 5e 怪物图鉴2014，
    source=MM25 查 5r 版），同名不同版本并存时用于精确打开指定词条。
    """
    q = name.strip()
    q_lower = q.lower()
    src_filter = request.args.get('source', '').strip()
    def _src_ok(src):
        return (not src_filter) or (str(src or '') == src_filter)
    custom_monsters = _load_custom('monsters')

    def _custom_payload(e):
        src = e.get('source', '自定义')
        return {
            'name': e['name'],
            'name_en': e.get('name_en', ''),
    ...
    # ━━ 第 1 轮：精确匹配（自定义 → CHM → SRD）━━
    # 1. 自定义怪物（中文名或英文名精确）
    for e in custom_monsters:
        if (e['name'].lower() == q_lower
                or (e.get('name_en') or '').lower() == q_lower) \
                and _src_ok(e.get('source', '自定义')):
            return jsonify(_custom_payload(e))

    # 2. CHM 完整详情（含 detail_text）——仅当命中条目与查询名精确一致
    #    时采用，模糊命中（如查询"地精"命中"地精喽"）不采用并继续；
    #    指定 source 时在同名多版本（5e MM / 5r MM25）中精确选版
    chm_results = chm_search_monster(q)
    if chm_results:
        m_exact = None
        if src_filter:
            for m in chm_results:
                if _monster_exact_hit(m, q_lower) and _src_ok(m.get('source', '')):
                    m_exact = m
                    break
        elif _monster_exact_hit(chm_results[0], q_lower):
            m_exact = chm_results[0]
        if m_exact is not None:
            monster = chm_get_monster_detail(q)
            # 指定版本时 get_monster_detail 可能取到其他版本条目，
            # 改为直接读取该条目自身 detail_link 的详情
            if src_filter or not monster:
                detail_text = ''
                if m_exact.get('detail_link'):
                    from core.chm_search import read_detail_page
                    detail_text = read_detail_page(m_exact['detail_link']) or ''
                monster = {**m_exact, 'detail_text': detail_text}
            if monster:
    ...
    # 3. SRD 精确匹配（srd_monsters.json，5e 2014 规则）
    if _src_ok('SRD'):
        srd_monster = _srd_monster_exact(q_lower)
        if srd_monster:
            srd_monster.setdefault('source', 'SRD')
            srd_monster.setdefault('rule_version', _rule_version_of('SRD'))
            return jsonify({**srd_monster, 'source_db': 'srd'})

    # ━━ 第 2 轮：模糊匹配兜底（全部数据源精确失败后才使用）━━
    # 4. 自定义怪物包含匹配（取名称最短者，最接近查询词）
    best = None
    for e in custom_monsters:
        if (q_lower in e['name'].lower() or q_lower in (e.get('name_en') or '').lower()) \
                and _src_ok(e.get('source', '自定义')):
            if best is None or len(e['name']) < len(best['name']):
                best = e
    if best:
        return jsonify(_custom_payload(best))

    # 5. CHM 模糊搜索（评分排序第一，指定 source 时取该版本首个匹配）
    if chm_results:
        m = chm_results[0]
        if src_filter:
            for x in chm_results:
                if _src_ok(x.get('source', '')):
                    m = x
                    break
        detail_text = ''
    ...
    # 6. SRD 包含匹配兜底
    if _src_ok('SRD'):
        monster = search_monster(q)
        if monster:
            monster.setdefault('source', 'SRD')
            monster.setdefault('rule_version', _rule_version_of('SRD'))
...
def api_create_from_monster():
    ...
    怪物真实数据（HP/AC/六属性）优先：请求字段（SRD 怪物）→ 词条文本
    解析（CHM 怪物，如 "AC 11...HP 19...力量15..."）→ CR 估算兜底。
    """
    data = request.get_json()
    name = data.get('name', '')
    if not name:
    ...
    # ━━ 真实怪物数据优先（字段 → 词条文本解析 → 估算兜底）━━
    detail_text = data.get('detail_text', '')
    src_text = detail_text or ''

    def _first_int(value, pattern=None):
    ...
    # HP：请求字段（"7 (2d6)" → 7）→ 词条文本（"HP 19（3d8+6）" → 19）→ 估算
    hp_real = _first_int(data.get('hp'))
    if hp_real is None:
        hp_real = _first_int(src_text, r'HP\s*(\d+)')
    hp = hp_real if hp_real is not None else (max(10, int(cr * 15 + 10)) if cr > 0 else 10)

    # ━━ HP 骰子表达式：按词条均值后括号内的"几d几+几"自动投掷生成 ━━
    hp_dice = ''
    if data.get('hp'):
        m = _re.search(r'[（(]\s*([^（）()]+?)\s*[）)]', str(data.get('hp')))
        if m:
            hp_dice = m.group(1)
    if not hp_dice and src_text:
        m = _re.search(r'HP\s*[：:]?\s*\d+\s*[（(]\s*([^（）()]+?)\s*[）)]', src_text, _re.I)
        if m:
            hp_dice = m.group(1)
    hp_dice = _re.sub(r'\s+', '', hp_dice)
    if not _re.fullmatch(r'\d*d\d+([+-]\d+)?', hp_dice, _re.I):
        hp_dice = ''
    hp_rolled = None
    if hp_dice:
        try:
            hp_rolled = max(1, roll(hp_dice).total)
        except Exception:
            hp_rolled = None
        if hp_rolled is not None:
            hp = hp_rolled

    # AC：请求字段 → 词条文本（"AC 15先攻..." / "AC 11"）→ 估算
    ac_real = _first_int(data.get('ac'))
    if ac_real is None:
        ac_real = _first_int(src_text, r'AC\s*(\d+)')
    ac = ac_real if ac_real is not None else (min(22, 10 + int(cr / 2) + 3) if cr > 0 else 13)

    # 属性：请求 abilities（SRD 字段）→ 词条文本六项（"力量15+2+2"）→ 估算
    abilities = data.get('abilities')
    abilities_real = False
    if isinstance(abilities, dict) and 'str' in abilities:
        try:
            abilities = {k: int(abilities[k]) for k in ('str', 'dex', 'con', 'int', 'wis', 'cha')}
            abilities_real = True
        except (TypeError, ValueError, KeyError):
            abilities = None
    if abilities is None:
        parsed = {}
        for key, cn in (('str', '力量'), ('dex', '敏捷'), ('con', '体质'),
                        ('int', '智力'), ('wis', '感知'), ('cha', '魅力')):
            v = _first_int(src_text, cn + r'\s*(\d+)')
            if v is not None:
                parsed[key] = v
        if len(parsed) == 6:
            abilities = parsed
    ...
    # 设置 HP 和 AC
    update_character(char_id, hp_max=hp, hp_current=hp, ac=ac)
    ...
    if detail_text:
        ...
        if hp_real: info_parts.append(f"HP：{hp_real}{f'（{hp_dice}）' if hp_dice else ''}")
        if ac_real: info_parts.append(f"AC：{ac_real}")
        if info_parts:
            bg_parts.append(f"【怪物数据】\n{' | '.join(info_parts)}")
            tag = '属性' if abilities_real else '估算属性'
            bg_parts.append(f"{tag}：力{abilities['str']} 敏{abilities['dex']} 体{abilities['con']} 智{abilities['int']} 感{abilities['wis']} 魅{abilities['cha']}")
...
def _prune_stale_users():
    """清理过期在线记录（超过 ROOM_HEARTBEAT_TIMEOUT 无心跳视为离线），并发送退出消息。"""
    global _dm_name, _dm_user_id, _dm_ip
    _now = _time.time()
    stale_names = [n for n, u in _online_users.items() if _now - u.get('last_heartbeat', 0) > ROOM_HEARTBEAT_TIMEOUT]
...
def api_room_users():
    """在线玩家列表（只读）：先清理过期用户，再返回当前在线玩家。

    供聊天室"🔄 刷新在线"按钮使用——未加入聊天（无心跳）也能查看在线列表，
    挂机玩家（页面仍打开、有心跳）会被保留显示。
    """
    global _online_users
    _prune_stale_users()
    return jsonify({
...
def _view_count_map(ctype: str, ids: list) -> dict:
    """内容浏览量映射 {id: count}（去重：每用户每内容仅计一次，v5.8）。"""
    if not ids:
        return {}
    try:
        from core.models import ContentView as _CV
        rows = (db.session.query(_CV.content_id, db.func.count())
                .filter(_CV.content_type == ctype, _CV.content_id.in_(ids))
                .group_by(_CV.content_id).all())
        return {r[0]: int(r[1]) for r in rows}
    except Exception:
        return {}


def _record_view(ctype: str, cid: int):
    """记录一次浏览（登录用户去重；未登录不统计）。幂等。"""
    user = _get_current_user()
    if not user:
        return jsonify({'ok': True, 'first': False})
    try:
        from core.models import ContentView as _CV
        exists = _CV.query.filter_by(content_type=ctype, content_id=cid,
                                     user_id=user.id).first()
        if exists:
            return jsonify({'ok': True, 'first': False})
        db.session.add(_CV(content_type=ctype, content_id=cid, user_id=user.id,
                           created_at=_time.strftime('%Y-%m-%d %H:%M:%S')))
        db.session.commit()
...
def api_community_threads(board_id):
    ...
    sort = (request.args.get('sort') or '').strip().lower()  # hot=浏览量降序（v5.8）
    user = _get_current_user()
    with _community_lock:
        posts = _load_json_file(_COMMUNITY_FILE, [])
    ...
    # 浏览量（去重统计）与热门排序（分页前应用）
    view_map = _view_count_map('post', [p.get('id') for p in threads])
    if sort == 'hot':
        threads.sort(key=lambda p: (-view_map.get(p.get('id'), 0), -(p.get('id') or 0)))
    total = len(threads)
    has_more = False
    if page is not None:
        page = max(page, 1)
    ...
    for o in out:
        o['views'] = view_map.get(o.get('id'), 0)
...
def api_friend_list():
    ...
    # 好友在线状态：复用全站在线会话（按 username 匹配，与在线系统一致）
    with _online_lock:
        _online_names = {s['username'] for s in _online_sessions.values()}
    friends = []
    for r in rows:
        other_id = r.target_id if r.user_id == user.id else r.user_id
        info = _friend_to_dict(other_id, user.id)
        if info:
            info['online'] = info.get('username') in _online_names
            friends.append(info)
...
@app.route('/api/community/posts/<int:post_id>/view', methods=['POST'])
def api_post_view(post_id):
    """帖子浏览计数（登录用户去重，v5.8）。"""
    return _record_view('post', post_id)


@app.route('/api/workshop/items/<int:item_id>/view', methods=['POST'])
def api_workshop_view(item_id):
    """工坊投稿浏览计数（登录用户去重，v5.8）。"""
    return _record_view('workshop', item_id)
...
def api_community_post_detail(post_id):
    ...
    enriched['views'] = _view_count_map('post', [post_id]).get(post_id, 0)
...
def api_workshop_items():
    ...
    # 浏览量（去重统计）与热门排序（分页前应用）
    view_map = _view_count_map('workshop', [s.get('id') for s in subs])
    if sort == 'hot':
        subs.sort(key=lambda s: (-view_map.get(s.get('id'), 0), -(s.get('id') or 0)))
    total = len(subs)
    has_more = False
    if page is not None:
        page = max(page, 1)
...
def api_workshop_item_detail(item_id):
    ...
    user = _get_current_user()
    fav_map = _fav_count_map()
    enriched = _enrich_workshop_item(item, fav_map, user)
    enriched['views'] = _view_count_map('workshop', [item_id]).get(item_id, 0)
...
# ═══════════════════════════════════════════════════════════════
# 模组时间 API（2026-09-01）：全团共享时历，DM 设置与手动推进
# ═══════════════════════════════════════════════════════════════
from core.models import CampaignTime as _CampaignTime

def _campaign_time_dict(row):
    return {
        'era': row.era or '圣历', 'year': row.year or 1, 'month': row.month or 1,
        'day': row.day or 1, 'hour': row.hour or 0, 'minute': row.minute or 0,
        'minutes_per_hour': row.minutes_per_hour or 60,
        'hours_per_day': row.hours_per_day or 24,
        'days_per_month': row.days_per_month or 30,
        'months_per_year': row.months_per_year or 12,
        'updated_by': row.updated_by or '', 'updated_at': row.updated_at or '',
    }

@app.route('/api/campaign-time', methods=['GET'])
def api_campaign_time_get():
    row = db.session.query(_CampaignTime).first()
    if not row:
        # 未设置/已清空 → 空状态（前端显示"未设置模组时间"）
        return jsonify({'empty': True, 'era': '', 'year': None, 'month': None, 'day': None,
                        'hour': None, 'minute': None, 'minutes_per_hour': None,
                        'hours_per_day': None, 'days_per_month': None, 'months_per_year': None,
                        'updated_by': '', 'updated_at': ''})
    return jsonify(_campaign_time_dict(row))

@app.route('/api/campaign-time', methods=['POST'])
def api_campaign_time_set():
    """设置起始时间（set 模式）、推进时间（advance 分钟数）或清空（clear）；仅 DM"""
    data = request.get_json(silent=True) or {}
    name = (data.get('name') or '').strip()
    if not _is_dm_request(name, request.remote_addr or ''):
        return jsonify({'error': '仅 DM 可设置模组时间'}), 403
    row = db.session.query(_CampaignTime).first()
    if not row:
        row = _CampaignTime()
        db.session.add(row)
    if data.get('clear'):
        ...
        if row:
            ...
            db.session.commit()
        return jsonify({'empty': True, 'ok': True})
    if data.get('set'):
        row.era = (data.get('era') or row.era or '圣历').strip()[:20]
        row.year = max(0, int(data.get('year') if data.get('year') is not None else (row.year or 1)))
        row.month = max(1, min(12, int(data.get('month') if data.get('month') is not None else (row.month or 1))))
        row.day = max(1, min(30, int(data.get('day') if data.get('day') is not None else (row.day or 1))))
        row.hour = max(0, min(23, int(data.get('hour') if data.get('hour') is not None else (row.hour or 0))))
        row.minute = max(0, min(59, int(data.get('minute') if data.get('minute') is not None else (row.minute or 0))))
        # 历法进位配置（可编辑，与模组历法一致）
        cfg = {
            'minutes_per_hour': (60, 1, 999), 'hours_per_day': (24, 1, 999),
            'days_per_month': (30, 1, 999), 'months_per_year': (12, 1, 999),
        }
        for key, (default, lo, hi) in cfg.items():
            v = data.get(key)
            if v is not None:
                setattr(row, key, max(lo, min(hi, int(v))))
            elif getattr(row, key, None) is None:
                setattr(row, key, default)
    elif data.get('advance'):
        # 推进 N 分钟（按可配置历法进位：分钟→时→日→月→年）
        mph = max(1, row.minutes_per_hour or 60)
        hpd = max(1, row.hours_per_day or 24)
        dpm = max(1, row.days_per_month or 30)
        mpy = max(1, row.months_per_year or 12)
        total = (row.minute or 0) + int(data.get('advance'))
        row.hour = (row.hour or 0) + total // mph
        row.minute = total % mph
        row.day = (row.day or 1) + row.hour // hpd
        row.hour = row.hour % hpd
        row.month = (row.month or 1) + (row.day - 1) // dpm
        row.day = ((row.day - 1) % dpm) + 1
        row.year = (row.year or 1) + (row.month - 1) // mpy
        row.month = ((row.month - 1) % mpy) + 1
    else:
        return jsonify({'error': '缺少 set 或 advance 参数'}), 400
    row.updated_by = name
    row.updated_at = _time.strftime('%Y-%m-%d %H:%M')
    db.session.commit()
    return jsonify(_campaign_time_dict(row))


# ═══════════════════════════════════════════════════════════════
# 线索与委托笔记 API（2026-09-01）：DM/PL 均可编辑，可公开或私有
# ═══════════════════════════════════════════════════════════════
from core.models import Note as _Note

def _note_dict(n):
    return {
        'id': n.id, 'kind': n.kind, 'title': n.title, 'description': n.description,
        'source': n.source, 'npc': n.npc, 'reward': n.reward, 'status': n.status,
        'visibility': n.visibility or 'public', 'owner_id': n.owner_id,
        'created_by': n.created_by or '', 'created_at': n.created_at or '', 'updated_at': n.updated_at or '',
    }

def _can_edit_note(n, user):
    """公开笔记全团可编辑；私有笔记仅创建者"""
    if n.visibility != 'private':
        return True
    return user is not None and n.owner_id == user.id

@app.route('/api/notes', methods=['GET'])
def api_notes_list():
    user = _get_current_user()
    if not user:
        return jsonify({'error': '请先登录（官网账号）后再使用笔记'}), 401
    kind = request.args.get('kind', '')
    q = db.session.query(_Note)
    # 公开 + 自己的私有
    q = q.filter(db.or_(_Note.visibility != 'private', _Note.owner_id == user.id))
    if kind:
        q = q.filter(_Note.kind == kind)
    rows = q.order_by(_Note.updated_at.desc()).all()
    return jsonify([_note_dict(n) for n in rows])
...
def api_notes_create():
    ...
    kind = data.get('kind') if data.get('kind') in ('clue', 'quest') else 'clue'
    vis = data.get('visibility') if data.get('visibility') in ('public', 'private') else 'public'
    n = _Note(
        kind=kind, title=title[:100],
        description=(data.get('description') or '')[:2000],
        source=(data.get('source') or '')[:100],
        npc=(data.get('npc') or '')[:100],
        reward=(data.get('reward') or '')[:100],
        status=(data.get('status') or '')[:20],
        visibility=vis,
        owner_id=user.id if vis == 'private' else None,
    ...
    db.session.add(n)
    db.session.commit()
    return jsonify(_note_dict(n)), 201
...
def api_notes_update(nid):
    ...
    n = db.session.get(_Note, nid)
    if not n:
        return jsonify({'error': '笔记不存在'}), 404
    if not _can_edit_note(n, user):
        return jsonify({'error': '无权编辑该私有笔记'}), 403
    data = request.get_json(silent=True) or {}
    if 'title' in data:
        ...
        if not t:
            return jsonify({'error': '标题不能为空'}), 400
        n.title = t[:100]
    for f in ('description', 'source', 'npc', 'reward', 'status'):
        if f in data:
            setattr(n, f, (data.get(f) or '')[:2000 if f == 'description' else 100])
    if data.get('kind') in ('clue', 'quest'):
        n.kind = data['kind']
    if data.get('visibility') in ('public', 'private'):
        n.visibility = data['visibility']
        n.owner_id = user.id if data['visibility'] == 'private' else None
    n.updated_at = _time.strftime('%Y-%m-%d %H:%M')
    db.session.commit()
...
def api_notes_delete(nid):
    ...
    if not n:
        return jsonify({'error': '笔记不存在'}), 404
    if not _can_edit_note(n, user):
        return jsonify({'error': '无权删除该私有笔记'}), 403
    db.session.delete(n)
    db.session.commit()
    return jsonify({'ok': True})
...
# ═══════════════════════════════════════════════════════════════
# 跑团页面登录拦截（2026-09-01）：未登录访问跑团平台/探索北境 →
# 重定向官网并提示登录；防止直接通过网址进入跳过官网登录
# ═══════════════════════════════════════════════════════════════
_LOGIN_REQUIRED_PAGES = {
    '/map', '/chat', '/dice3d', '/dice-help', '/character', '/combat',
    '/spells', '/events', '/reference', '/mods', '/north-expedition',
}


@app.before_request
def _login_gate():
    """未登录用户访问跑团页面 → 官网首页 ?need_login=1（官网弹登录提示）"""
    if request.method != 'GET':
        return None
    if request.path not in _LOGIN_REQUIRED_PAGES:
        return None
    if _get_current_user():
        return None
    return redirect('/?need_login=1')
