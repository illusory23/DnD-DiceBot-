...
_chat_messages: list[dict] = []  # [{name, text, time, is_dm, ip}]
MAX_CHAT_MSGS = 200  # 最多保留200条消息
...
def api_check():
    ...
    return jsonify({
        ...
        'd20_rolls': d20_rolls,  # 所有 d20 结果（优势/劣势时显示用）
...
def _is_dm_ip(ip: str) -> bool:
    """Check if the IP belongs to the DM (localhost or server's own IP)."""
    if ip in ('127.0.0.1', 'localhost', '::1'):
        return True
    # Check if it matches the server's LAN IP
    try:
        hostname = _os.popen('hostname').read().strip()
        local_ip = _os.popen('ipconfig 2>nul | findstr /i "IPv4"').read()
        if ip in local_ip:
            return True
    except Exception:
        pass
