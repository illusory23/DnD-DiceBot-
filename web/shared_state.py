    ...
    try:
        if _SAVE_FILE.exists():
            ...
            if isinstance(data, dict):
                ...
                return canvas, float(data.get('_ts', 0.0)), int(data.get('_ver', 0)), migrated
    except Exception as e:
        print(f'[shared_state] 读取 {_SAVE_FILE.name} 失败: {e}')
...
def _save_to_disk():
    ...
    try:
        ...
        print(f'[shared_state] 保存 {_SAVE_FILE.name} 失败: {e}')
